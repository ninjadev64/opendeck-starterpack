...
impl openaction::ActionEventHandler for ActionEventHandler {
	async fn key_down(&self, event: KeyEvent, _outbound: &mut openaction::OutboundEventManager) -> EventHandlerResult {
		match &event.action[..] {
			...
			"com.amansprojects.starterpack.presskeys" => press_keys::key_down(event),
			_ => Ok(()),
		}
	}

	async fn key_up(&self, event: KeyEvent, _outbound: &mut openaction::OutboundEventManager) -> EventHandlerResult {
		match &event.action[..] {
			"com.amansprojects.starterpack.runcommand" => run_command::key_up(event),
			"com.amansprojects.starterpack.presskeys" => press_keys::key_up(event),
