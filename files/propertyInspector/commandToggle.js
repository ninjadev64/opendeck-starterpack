function connectElgatoStreamDeckSocket(inPort, inPropertyInspectorUUID, inRegisterEvent, inInfo, inActionInfo) {
	...
	s1_down.value = inActionInfo.payload.settings.s1_down ?? "";
	let s1_up = document.getElementById("s1up");
	s1_up.value = inActionInfo.payload.settings.s1_up ?? "";
	let s2_down = document.getElementById("s2down");
	s2_down.value = inActionInfo.payload.settings.s2_down ?? "";
	let s2_up = document.getElementById("s2up");
	s2_up.value = inActionInfo.payload.settings.s2_up ?? "";
